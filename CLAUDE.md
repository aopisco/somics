...
| **seqFISH (run 3, launched 2026-09-11 13:33Z, `somics-seqfish-3`, `i-0ecef8b3411393c44`)** | `ingest/seqfish/atlas/2026-09-11T13-*` | 6 HuBMAP Cai-lab datasets as 43 FOV sections with DAPI. Runs 1-2 skipped all six on the malformed ImageJ DAPI stacks (empty pages, then "incompatible keyframe"); the builder now reads the planes contiguously in the file's big-endian byte order, validated on the real stack with a centroid overlay (`docs/figures/seqfish_hbm782_fov0_dapi_centroids.png`). Runs 1-2 prefixes deleted |
