...
| **seqFISH (run 5, launched 2026-09-11 18:02Z, `somics-seqfish-5`, `i-0f48ea32ba8839878`)** | `ingest/seqfish/atlas/2026-09-11T18-*` | 6 HuBMAP Cai-lab datasets as 43 FOV sections with DAPI. Runs 1-4 added nothing (prefixes deleted): malformed ImageJ DAPI stacks (empty pages; "incompatible keyframe"; big-endian) -> builder reads planes contiguously in the file's byte order, validated on the real stack (`docs/figures/seqfish_hbm782_fov0_dapi_centroids.png`); then a verbatim-duplicated gene row (EEF2) broke the harmonizer's keyed merge -> deduplicated, blanks kept |
